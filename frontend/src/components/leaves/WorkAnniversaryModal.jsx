...
export default function WorkAnniversaryModal({ anniversaries, currentUser }) {
  ...
  useEffect(() => {
    ...
    let unseen = anniversaries.find((w) => {
      ...
      const key = `work_anniversary_dismissed_${currentYear}_${currentUser?.id}_${w.employee_code}`
  ...
  const dismiss = () => {
    const currentYear = new Date().getFullYear()
    const key = `work_anniversary_dismissed_${currentYear}_${currentUser?.id}_${activeAnniversary.employee_code}`
