...
export default function PlatformHelpdeskPage() {
    const confirm = useConfirm();
    const { isPrivileged, user } = useAuth(); // HR/Admin
    ...
    const fetchTickets = async () => {
        try {
            const res = await helpdeskApi.getPlatformTickets();
    ...
    const handleDeleteTicket = async (e, id) => {
        ...
        try {
            await helpdeskApi.deletePlatformTicket(id);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await helpdeskApi.createPlatformTicket(data);
    ...
    const filteredTickets = tickets.filter(t => {
        ...
        if (searchTerm) {
            ...
            return t.title.toLowerCase().includes(lower) || 
                   t.created_by_name.toLowerCase().includes(lower) || 
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="mb-4 sm:mb-0">
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
                        <LifeBuoy className="w-6 h-6 text-indigo-600" /> Platform Support
                    </h1>
                ...
                {loading ? (
                    ...
                    <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
                        {filteredTickets.map(ticket => (
                            <Link 
                                key={ticket.id} 
                                to={user?.is_superuser ? `/platform/support/${ticket.id}` : `/platform-support/${ticket.id}`}
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="mt-2 flex items-center gap-4 text-sm text-slate-500 dark:text-slate-400">
                                            <div className="flex items-center gap-1.5">
                                                <div className="w-5 h-5 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-xs font-bold text-slate-600 dark:text-slate-300">
                                            ...
                                            {user?.is_superuser && ticket.organization_name && (
                                                <>
                                                    ...
                                                    <span className="font-medium text-slate-700 dark:text-slate-300 truncate max-w-[150px]" title={ticket.organization_name}>
