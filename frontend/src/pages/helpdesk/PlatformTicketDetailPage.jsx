...
export default function PlatformTicketDetailPage() {
    ...
    const fetchTicket = async () => {
        try {
            const res = await helpdeskApi.getPlatformTicket(id);
            setTicket(res.data);
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            await helpdeskApi.createPlatformMessage(data);
            setNewMessage('');
            setAttachment(null);
            fetchTicket();
        } catch (error) {
    ...
    const handleDeleteMessage = async (messageId) => {
        ...
        try {
            await helpdeskApi.deletePlatformMessage(messageId);
            setTicket({
                ...ticket,
    ...
    const handleStatusChange = async (newStatus) => {
        if (!isPrivileged || isUpdatingStatus) return;
        setIsUpdatingStatus(true);
        try {
            await helpdeskApi.updatePlatformTicketStatus(id, newStatus);
    ...
    return (
        <div className="max-w-7xl mx-auto h-[calc(100vh-6rem)] flex flex-col md:flex-row gap-6">
            ...
            <div className="w-full md:w-1/3 flex flex-col gap-4 overflow-y-auto pr-2 pb-4">
                <Link to="/platform-support" className="inline-flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-indigo-600 transition-colors w-max mb-2">
                    <ArrowLeft className="w-4 h-4" /> Back to Platform Support
                </Link>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm">
                    ...
                    <div className="flex items-center gap-3 mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                        {ticket.created_by_avatar ? (
                            <img src={ticket.created_by_avatar} alt="" className="w-10 h-10 rounded-full object-cover border-2 border-white dark:border-slate-800 shadow-sm" />
                        ) : (
                        ...
                        <div>
                            <div className="font-semibold text-sm text-slate-900 dark:text-slate-100">
                                ...
                                {user?.is_superuser && ticket.organization_name && (
                                    <span className="text-slate-500 font-normal"> ({ticket.organization_name})</span>
                                )}
                            </div>
                            <div className="text-xs text-slate-500">{ticket.employee_code || 'Admin'}</div>
                        </div>
                    </div>
            ...
            <div className="w-full md:w-2/3 flex flex-col bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden h-full">
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 bg-slate-50/30 dark:bg-slate-900/30">
                    ...
                    <div className="flex gap-4">
                        {ticket.created_by_avatar ? (
                            <img src={ticket.created_by_avatar} alt="" className="w-8 h-8 rounded-full object-cover shrink-0 mt-1" />
                        ) : (
                        ...
                        <div className="flex-1 max-w-[85%]">
                            <div className="flex items-baseline gap-2 mb-1">
                                <span className="font-bold text-sm text-slate-800 dark:text-slate-200">{ticket.created_by_name}</span>
                        ...
                        return (
                            <div key={msg.id} className={`flex gap-4 ${isMe ? 'flex-row-reverse' : ''}`}>
                                {msg.sender_avatar ? (
                                    ...
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 mt-1 ${isMe ? 'bg-indigo-600 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300'}`}>
                                        {msg.sender_name.charAt(0)}
                                    </div>
                                )}
                                <div className={`flex-1 max-w-[85%] flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                                    <div className={`flex items-baseline gap-2 mb-1 ${isMe ? 'flex-row-reverse' : ''}`}>
                                        <span className="font-bold text-sm text-slate-800 dark:text-slate-200">{msg.sender_name}</span>
                                    ...
                                    <div className={`p-3.5 rounded-2xl shadow-sm text-sm whitespace-pre-wrap max-w-full break-words
                                        ${isMe ? 'bg-indigo-600 text-white rounded-tr-sm' : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-100 dark:border-slate-700 rounded-tl-sm'}
                                    `}>
                                        {msg.message}
                                        {msg.attachment && (
