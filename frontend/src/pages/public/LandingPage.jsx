...
export default function LoginPage() {
  ...
  // Restore auto-redirect for logged-in users to redirect to dashboard
  useEffect(() => {
    if (user && !authLoading) {
      const dest = user.is_superuser && !user.organization_id ? '/platform' : '/dashboard'
  ...
  // Removed early return for auth loading so landing page always renders
  ...
  return (
    <div className="min-h-screen font-sans bg-slate-50 dark:bg-[#0b0f19] text-slate-900 dark:text-white overflow-x-hidden selection:bg-brand-500/30">
      ...
      {/* Modern OS Section */}
      <section className="py-24 relative overflow-hidden bg-white dark:bg-[#0b0f19]">
        <div className="absolute inset-0 z-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-slate-100 dark:from-slate-800/20 via-white dark:via-[#0b0f19] to-white dark:to-[#0b0f19] opacity-50" />
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 text-center">
          ...
          <h2 className="text-5xl md:text-7xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-8">
            The Modern OS for your <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-500 to-indigo-600">Global Workforce</span>
          </h2>
          
          <p className="text-xl text-slate-600 dark:text-slate-400 mb-12 max-w-3xl mx-auto leading-relaxed">
            Automate payroll, streamline attendance, and empower your employees with a premium multi-tenant platform built for scale.
          </p>
          
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to="/demo" className="px-8 py-4 rounded-full bg-teal-600 text-white font-semibold flex items-center gap-2 hover:bg-teal-700 transition-colors w-full sm:w-auto justify-center">
              Request a Demo
              <ArrowRight className="w-4 h-4" />
            </Link>
            <Link to="/pricing" className="px-8 py-4 rounded-full bg-white border border-slate-200 text-slate-900 font-semibold hover:bg-slate-50 transition-colors w-full sm:w-auto justify-center">
              View Pricing
            </Link>
          </div>
        </div>
